...
**ai-kb user auth (migration 0007).** Beyond admin tokens, `ai-kb/src/auth.js` implements username + password login backed by the `users` and `user_sessions` tables. PBKDF2-SHA256, 100k iterations, 16-byte salt; session token is 32 random bytes hex sent as `Authorization: Bearer <token>` and lives 30 days. After login, the server uses `user.id` as the effective owner id when reading/writing chats; before login, anonymous browsers use a `client_id` UUID stored in `localStorage` (migration 0006 added the `chat_sessions.client_id` column). `GET /api/sessions?client_id=…` is therefore **no longer admin-gated** — it returns only that client's (or that user's) chats. Admin tokens still let you list all sessions without a filter.
...
2. Runs retrieval legs in parallel:
   ...
   - Geometric d×D×B match via `bearings.js` if the message contains a `NN×NN×NN` triple.
...
- **Per-client chats** (ai-kb `0006`) — `chat_sessions.client_id` column + `idx_sessions_client (client_id, updated_at DESC)`. Lets `GET /api/sessions?client_id=…` return a single browser's history without admin auth.
